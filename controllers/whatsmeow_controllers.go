...
func ProcessMessage(v *events.Message) {
	...
	if len(senderPhoneNumber) > 12 && !strings.HasPrefix(senderPhoneNumber, "62") && !strings.HasPrefix(senderPhoneNumber, "0") {
		...
		if v.Info.Sender.Server == "lid" {
			...
			// Method 1: Try to find in message history by chat_jid
	...
	if err != nil {
		...
		if err.Error() == config.GetConfig().Whatsmeow.WaErrorMessage.ID.PhoneNumberNotRegistered ||
			...
			if v.Info.IsGroup {
				...
				groupInfo, groupErr := WhatsappClient.GetGroupInfo(v.Info.Chat)
				if groupErr != nil {
	...
	result := CheckPromptPermission(v, messageTextLower, sanitizeRes.User, userLang)
	if result.Allowed {
...
func ValidateGroupJID(groupJID string) (types.JID, error) {
	...
	if groupJID == "" {
		return types.JID{}, fmt.Errorf("invalid group JID format: empty group ID")
	}
...
func handleKeywordReply(jid, messageText, messageTextLower, userLang string, sanitizeRes *SanitizationResult) {
	if sanitizeRes != nil {
		...
		if exists == 0 {
			...
			// 	// TODO: customize welcome message for general user if needed
			// }
...
func CheckWAPhoneNumberIsRegistered() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(phoneNumber) > digitNoTelp {
			...
			result, err := WhatsappClient.IsOnWhatsApp([]string{sanitizedPhoneNumber + "@s.whatsapp.net"})
			if err != nil {
