...
func CheckValidWhatsappPhoneNumber(phoneNumber string) (string, error) {
	...
	results, err := WhatsappClient.IsOnWhatsApp([]string{jid})
	if err != nil {
...
func sendTextMessageViaBot(jid string, message string) {
	...
	_, err = WhatsappClient.SendMessage(context.Background(), userJID, &waProto.Message{
...
func PingWhatsapp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		msgToSend := "Ping!"
		resp, err := WhatsappClient.SendMessage(context.Background(), types.NewJID("62"+validPhoneNumber, "s.whatsapp.net"), &waProto.Message{
...
func SendTextWhatsapp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		resp, err := WhatsappClient.SendMessage(context.Background(), jid, &waProto.Message{
	...
	return func(c *gin.Context) {
		...
		for _, group := range groups {
			...
			for _, p := range group.Participants {
				...
				userInfos, _ := WhatsappClient.GetUserInfo([]types.JID{jid})
				userInfo := userInfos[jid]

				avatarInfo, _ := WhatsappClient.GetProfilePictureInfo(jid, &whatsmeow.GetProfilePictureParams{
					Preview: false,
				})
			...
			ownerInfos, _ := WhatsappClient.GetUserInfo([]types.JID{ownerJID})
			ownerInfo := ownerInfos[ownerJID]

			ownerAvatarInfo, _ := WhatsappClient.GetProfilePictureInfo(ownerJID, &whatsmeow.GetProfilePictureParams{
				Preview: false,
			})
...
func SendLocationWhatsapp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var msgToUse *waProto.Message
		if isLive == "true" { // Live location
			msgToUse = &waProto.Message{
...
// ADD if u need interactive msg btn
// func sendInteractiveMsgBtn() {
// 	msg := &waProto.Message{
...
// 					HydratedTitleText: "🍉 Choose a fruit",
// 				},
// 				HydratedContentText: proto.String("Select one of the fruits below:"),
// 				HydratedFooterText:  proto.String("Only one choice allowed."),
...
// 								DisplayText: proto.String("Apple 🍎"),
// 								ID:          proto.String("fruit_apple"),
// 							},
// 						},
// 					},
// 					{
// 						HydratedButton: &waProto.HydratedTemplateButton_QuickReplyButton{
// 							QuickReplyButton: &waE2E.HydratedTemplateButton_HydratedQuickReplyButton{
// 								DisplayText: proto.String("Banana 🍌"),
// 								ID:          proto.String("fruit_banana"),
// 							},
// 						},
// 					},
// 				},
...
// 		},
// 	}
// 	parsedJID, err := types.ParseJID(originalSenderJID)
// 	if err != nil {
// 		fmt.Printf("Failed to parse JID: %v\n", err)
// 		return
// 	}
// 	userJID := types.JID{
// 		User:   parsedJID.User,
...
// 	if err != nil {
// 		fmt.Printf("Failed to send interactive message: %v\n", err)
// 	}
...
func SendDocumentViaBotWhatsapp(jid, msg, filePath string) {
	...
	resp, err := WhatsappClient.SendMessage(context.Background(), userJID, &waProto.Message{
		DocumentMessage: &waProto.DocumentMessage{
