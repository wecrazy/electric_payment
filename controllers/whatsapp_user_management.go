			...
			for i := 0; i < t.NumField(); i++ {
				...
				case "user_type":
					...
					userTypeInfo := map[model.WAUserType]struct {
						...
						model.ClientUser:     {"fad fa-user-tie", "Client User"},
					}
				...
				case "user_of":
					...
					var htmlRendered string
					switch t {
					case model.UserOfPLTMH:
						htmlRendered = "PLTMH Lembang Palesan"
					case model.UserOfGeneral:
						htmlRendered = "General / Common User"
					}
					newData[theKey] = htmlRendered
				default:
					if fieldValue.Type() == reflect.TypeOf(time.Time{}) {
...
func sanitizeAndValidateInputWAUserManagement(data map[string]interface{}) (map[string]interface{}, error) {
	...
	if userTypeRaw, ok := data["user_type"]; ok && userTypeRaw != nil {
		...
		if !ok {
			return nil, fmt.Errorf("invalid type for user_type, must be string")
		}
	...
	if maxDailyQuotaRaw, ok := data["max_daily_quota"]; ok {
		...
		case string:
			if quota, err := strconv.Atoi(v); err == nil && quota > 0 {
				...
				return nil, fmt.Errorf("invalid max_daily_quota: must be a positive integer")
			}
		case float64:
			if int(v) > 0 {
				...
				return nil, fmt.Errorf("invalid max_daily_quota: must be > 0")
			}
		case int:
			if v > 0 {
		...
		default:
			return nil, fmt.Errorf("invalid type for max_daily_quota")
		}
	}
	...
	if isRegisteredRaw, ok := data["is_registered"]; ok {
		...
		case float64:
			isRegistered = v != 0.0
		default:
			return nil, fmt.Errorf("invalid type for is_registered, expected boolean")
		}
	...
	if allowedToCallRaw, ok := data["allowed_to_call"]; ok {
		...
		case float64:
			allowedToCall = v != 0.0
		default:
			return nil, fmt.Errorf("invalid type for allowed_to_call, expected boolean")
		}
	...
	if rawAllowedTypes, ok := data["allowed_types[]"]; ok {
		...
		if rawAllowedTypes == nil {
			return nil, fmt.Errorf("allowed_types cannot be nil")
		}

		switch v := rawAllowedTypes.(type) {
		case []string:
			if len(v) == 0 {
				return nil, fmt.Errorf("at least one allowed_type is required")
			}
			// Optional: Validate each element isn't empty
			for _, t := range v {
				if strings.TrimSpace(t) == "" {
					return nil, fmt.Errorf("allowed_types contains empty values")
				}
			}
		...
		case string:
			if strings.TrimSpace(v) == "" {
				return nil, fmt.Errorf("allowed_type cannot be empty string")
			}
			data["allowed_types"] = []string{v}

		default:
			return nil, fmt.Errorf("invalid type for allowed_types, expected []string or string")
		}

		delete(data, "allowed_types[]")
	} else {
		// If field is required but missing entirely
		return nil, fmt.Errorf("allowed_types is required")
	}
		...
		dummyValues := [][]string{
			{
				...
				fmt.Sprintf("e.g. : %s (Select 1 from [%s])", model.UserOfPLTMH, allowedUserOfList),
			},
			{
				"User Biasa",
				"biasa@aja.com",
		...
		for j := 0; j < modelType.NumField(); j++ {
			...
			case reflect.TypeOf(model.WAUserOf("")):
				...
				valid := map[model.WAUserOf]bool{
					model.UserOfPLTMH:   true,
					model.UserOfGeneral: true,
				}
				if !valid[candidate] {
