...
func (uc *UserClientWhatsapp) handleCommand(cmd ClientCommand) {
	...
	case CommandSendText:
		...
		if !ok {
			if cmd.Result != nil {
				cmd.Result <- fmt.Errorf("invalid payload type")
			}
			return
		}
...
func (uc *UserClientWhatsapp) sendText(payload SendTextPayload) error {
	...
	if !isAuthenticated {
		return fmt.Errorf("client not authenticated - please scan QR code first")
	}
	...
	// Send the message
	_, err := uc.Client.SendMessage(context.Background(), jid, &waProto.Message{
